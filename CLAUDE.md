...
Each `Application/Features/<Area>/` groups `Commands/<Verb><Entity>/` and `Queries/<Verb><Entity>/`, each folder holding the `I(Command|Query)` record plus its `internal` `I(Command|Query)Handler` (see [CreateGateCommandHandler.cs](src/Remote.Shell.Interrupt.Storehouse/Core/Remote.Shell.Interrupt.Storehouse.Application/Features/Gates/Commands/CreateGate/CreateGateCommandHandler.cs) for the pattern). [Mediator](https://github.com/martinothamar/Mediator) (martinothamar's source-generator library — **not** MediatR, which went commercial past v12; unrelated NuGet package despite the similar name) dispatches; `Behaviors/ValidationBehavior` and `Behaviors/LoggingBehavior` wrap every request. Handler `Handle` methods return `ValueTask<T>`, not `Task<T>` (the library's signature). Many create/update/delete handlers derive from generic base handlers (`CreateEntityCommandHandler<TEntity, TDto, TCommand>`, etc.) that take an `ISpecification<T>` for duplicate-checking and an `IMapper` (Mapster's `MapsterMapper.IMapper`) for DTO mapping — override the few hook methods (`BuildDuplicateCheckFilter`, `ValidateEntityDoesNotExistAsync`, `PersistNewEntity`) rather than reimplementing CRUD. Each DTO implements Mapster's `IRegister.Register(TypeAdapterConfig)` to declare its own mapping (see [GateDTO.cs](src/Remote.Shell.Interrupt.Storehouse/Core/Remote.Shell.Interrupt.Storehouse.Application/DTOs/Gates/GateDTO.cs)); `ApplicationServicesRegistration` scans the assembly for these at startup instead of a per-DTO manual registration list.

Two Mediator-specific quirks worth knowing before touching this layer:
- **`CQRS.` prefix** — this project's own `ICommand<T>`/`IQuery<T>`/`ICommandHandler<,>`/`IQueryHandler<,>` (in `Contracts/CQRS`) collide by name with Mediator's own built-in marker interfaces of the same names. `Application/GlobalUsing.cs` aliases the namespace (`global using CQRS = ...Contracts.CQRS;`, not a type alias — this compiler doesn't support C#12 generic using-aliases) instead of the namespace itself, so every command/query declares `: CQRS.ICommand<T>` / `: CQRS.IQuery<T>`, not the bare name.
- **Split packages, `InternalsVisibleTo`** — `Mediator.SourceGenerator` (an analyzer) is referenced only by the **API** project (the "edge" project, per the library's own convention), not Application; `Mediator.Abstractions` is referenced by Application, where the messages/handlers live. The generator writes generated dispatch code in the API project that names handler types directly (monomorphized `Send`, no reflection) — since handlers are `internal`, `Application/AssemblyInfo.cs` grants `[assembly: InternalsVisibleTo("Remote.Shell.Interrupt.Storehouse.API")]` for this. Mediator registration itself (`services.AddMediator(...)`, with `ServiceLifetime.Scoped` explicitly set — the library defaults to `Singleton`, which would capture the `AddScoped` `IUnitOfWork` implementations as captive dependencies) therefore lives in the API project's `ServiceRegistration.cs`, not `ApplicationServicesRegistration.cs`.
