...
/// </summary>
public sealed record UpdateUserProfileCommand(Guid UserId, string Email, string? FullName) : ICommand;

internal class UpdateUserProfileCommandHandler(IIdentityService identityService)
  : ICommandHandler<UpdateUserProfileCommand>
{
  async Task<Unit> IRequestHandler<UpdateUserProfileCommand, Unit>.Handle(UpdateUserProfileCommand request,
