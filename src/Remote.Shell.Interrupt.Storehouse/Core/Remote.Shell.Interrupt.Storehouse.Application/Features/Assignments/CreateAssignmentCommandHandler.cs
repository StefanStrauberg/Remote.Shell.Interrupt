  ...
  : ICommandHandler<CreateAssignmentCommand, Unit>
{
  ...
  {
    var existingAssignment = await _assignmentRepository.ExistsAsync(x => x.TargetFieldName == request.Name,
                                                                     cancellationToken);
    ...
    await _assignmentRepository.InsertOneAsync(assignment, cancellationToken);
