...
public class TfPlansController : BaseAPIController
{
    ...
    public async Task<IActionResult> GetTfPlans([FromQuery] RequestParameters requestParameters,
                                                CancellationToken cancellationToken)
    {
        var result = await Sender.Send(new GetTfPlansQuery(requestParameters),
                                       cancellationToken);
        var metadata = new
        {
            result.TotalCount,
